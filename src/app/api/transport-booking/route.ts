...
export async function POST(request: Request) {
  try {
    ...
    const emailSubject = `🚗 Nouvelle Réservation de Transfert : ${origin} → ${destination} - ${name}`
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #fff; padding: 30px; border-radius: 12px;">
          <div style="text-align: center; margin-bottom: 24px;">
            <h1 style="color: #c9a84c; font-size: 24px; margin: 0;">🚗 Nouvelle Réservation de Transfert</h1>
            <p style="color: #888; margin-top: 8px;">Be Flex Travel — Demande de Transfert Personnalisé</p>
          </div>

          <div style="background: #1a1a1a; border-radius: 8px; padding: 20px; margin-bottom: 16px;">
            <h2 style="color: #c9a84c; font-size: 16px; margin: 0 0 16px;">📍 Détails du Trajet</h2>
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 8px 0; color: #888; width: 40%;">Départ</td>
          ...
          <p style="color: #555; font-size: 12px; text-align: center; margin-top: 24px;">
            Cette demande a été soumise via le formulaire de réservation de transport du site Be Flex Travel.
          </p>
        </div>
      `,
    })
    ...
    const { error: clientEmailError } = await resend.emails.send({
      from: emailFrom,
      to: [email],
      subject: `✅ Transfert Confirmé — ${origin} → ${destination} | Be Flex Travel`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #0a0a0a; color: #fff; padding: 30px; border-radius: 12px;">
          <div style="text-align: center; margin-bottom: 28px;">
            <h1 style="color: #c9a84c; font-size: 26px; margin: 0;">🚗 Be Flex Travel</h1>
            <p style="color: #888; margin-top: 8px; font-size: 14px;">Votre demande de transfert a bien été reçue !</p>
          </div>
          ...
          <div style="background: linear-gradient(135deg, #c9a84c22, #1a1a1a); border: 1px solid #c9a84c44; border-radius: 10px; padding: 20px; margin-bottom: 20px; text-align: center;">
            ...
            <p style="color: #ccc; margin: 0; font-size: 14px;">Nous avons bien reçu votre demande de transfert. Notre équipe vous contactera très prochainement pour confirmer les détails et le tarif.</p>
          </div>
          ...
          <div style="background: #1a1a1a; border-radius: 8px; padding: 20px; margin-bottom: 16px;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr style="border-bottom: 1px solid #2a2a2a;">
                <td style="padding: 10px 0; color: #888; width: 45%;">Trajet</td>
                <td style="padding: 10px 0; color: #fff; font-weight: bold;">${origin} → ${destination}</td>
              </tr>
              <tr style="border-bottom: 1px solid #2a2a2a;">
    ...
    console.error('Erreur API Transport Booking:', error)
